...
One big we made for Quest was using the PSO caching process to cache ahead of time and package into the build. 
...
Needing to support such a vast difference in power between our target platforms, we needed to come up with an easy way to take advantage of the full power of each platform, and ensure that we delivered visuals consistent with the platform.
...
During devlopment, we also added a version of the game that could be played entirely in first person, without a VR headset, as well as a slightly less graphically demaning version for the Meta Quest.
...
- "Porting" features
    ...
    Working with design to identify intent, and build features to support that was the best path forward.
...
I baked a bunch of bread, crocheted beanies and blankets, tinkered with an ECS multiplayer project using Rust and Unity, and a whole lot more.
